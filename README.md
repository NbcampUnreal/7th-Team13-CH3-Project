...
### [유시환]몬스터 AI, 몬스터 웨이브
#### 1. 몬스터 사망 몽타주 재생 후 재추적(Zombie 현상) 오류
- **Problem:** 몬스터 사망 시, 사망 애니메이션 재생 후, 다시 걷기 애니메이션이 재생되면서 추적을 재시하는 현상 발생 
- **Cause:**
  1. **BT 로직의 지속성:** 사망 애니메이션 재생 중에도 비헤이비어 트리(BT)가 종료되지 않아, 다음 틱에서 상위 노드(Selector/Sequence)에 의해 추적 노드가 다시 실행됨.
  2. **관성 이동:** 이동 컴포넌트가 살아있어 사망 직후에도 물리적 관성에 의해 몬스터가 플레이어 쪽으로 미끄러지는 현상 동반.
- **Solution:**
  1. **이동 원천 차단:** StopMovementImmediately()와 Deactivate()를 호출하여 물리적 이동 기능을 즉시 마비시킴.
  2. **LifeSpan 예약:** 몽타주 재생 시간(PlayTime)을 계산하여, 애니메이션이 끝나기 직전(95% 시점)에 액터가 자동 파괴되도록 SetLifeSpan 설정.
  3. **로직 가두기:** 태스크의 반환값을 InProgress로 유지하여, 액터가 파괴되기 전까지 BT의 제어권이 다른 노드로 넘어가지 않도록 강제함.
- **Result:** 사망 애니메이션이 자연스럽게 출력된 후 로직 재발생 없이 액터가 정상적으로 제거됨을 확인.
